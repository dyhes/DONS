...
namespace Samples.DONSSystem
{
    ...
    public partial class BuildTopoSystem : SystemBase
    {
                    ...
                    .ForEach((Entity BuildEntity, int entityInQueryIndex, in BuildTopoConfig build, in DynamicBuffer<LinkDetail> link_table) =>
                    {
                        ...
                        if (targetEntities.Length == build.host_node && switchEntities.Length < build.switch_node)
                        {
                            ...
                            for (int i = 0; i < build.switch_node; i++)
                            {
                                ...
                                ecb.AddBuffer<ReceivedPacketEntry>(swEntity);
                                ecb.AddBuffer<TopoEntry>(swEntity);
                                ecb.AddBuffer<RoutingEntry>(swEntity);
                                ecb.AddBuffer<LSAIdentifier>(swEntity);
                                ecb.AddComponent(swEntity, new SwitchData
                                {
                                    switch_id = node_id,
                                    host_node = build.host_node,
                                    switch_node = build.switch_node,
        ...
        void StopBuildAndStartActionSystem()
        {
            ...
            var sys6 = World.DefaultGameObjectInjectionWorld?.GetExistingSystem<HelloSystem>();
            var sys7 = World.DefaultGameObjectInjectionWorld?.GetExistingSystem<ReceiveSystem>();
            var sys8 = World.DefaultGameObjectInjectionWorld?.GetExistingSystem<SPFSystem>();
